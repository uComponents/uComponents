...
namespace uComponents.Core.DataTypes.CheckBoxTree
{
	...
	public class CheckBoxTreeDataEditor : CompositeControl, IDataEditor
	{
		...
		private Dictionary<int, string> SelectedNodes
		{
			get
			{
				if (this.selectedNodes == null)
				{
					...
					var nodes = Xslt.Xml.CouldItBeXml(value) ? uQuery.GetNodesByXml(value) : uQuery.GetNodesByCsv(value);
		...
		public void Save()
		{
			...
			{
				this.data.Value = Xslt.Xml.Split(csv, new[] { comma }, "CheckBoxTree", "nodeId").OuterXml;
			}
			else
			{
				this.data.Value = csv;
