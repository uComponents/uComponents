...
namespace uComponents.Core.DataTypes.Similarity
{
	...
	public class SimilarityDataEditor : Panel
	{
		...
		public string SelectedNodes
		{
			...
			set
			{
				_selectedNodes = value; if (_selectedNodes != string.Empty)
				{
		...
		protected override void CreateChildControls()
		{
			...
			{
				ID = "FindSimilar",
				Text = SimilarityResources.SimilarityDataEditor_CreateChildControls_Find_similar
			};
		...
		private IEnumerable<SearchResultItem> FindSimilar()
		{
			...
			var more = new MoreUmbracoDocsLikeThis(CurrentDocId,
												   IndexToSearch,
												   MaxResults + 1,
												   PropertiesToSearch);
