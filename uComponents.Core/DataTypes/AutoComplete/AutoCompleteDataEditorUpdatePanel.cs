...
namespace uComponents.Core.DataTypes.AutoComplete
{
	...
	internal class AutoCompleteDataEditorUpdatePanel : UpdatePanel, IDataEditor
	{
		...
		{
			get { return uQuery.GetUmbracoObjectType(new Guid(this.options.TypeToPick)); }
		}
		...
		#region helpers
		private static Dictionary<int, string> BuildIntStringDictionary(string csv, uQuery.UmbracoObjectType umbracoObjectType)
		{
		...
		private static void BuildIntStringDictionary(string csv, uQuery.UmbracoObjectType umbracoObjectType, ref Dictionary<int, string> collection)
		{
		...
		private void BuildAutoCompleteAdd()
		{
			...
			string jsAutocomplete = @"  <script type='text/javascript'>
                                            jQuery(function(){
                                                jQuery('#" + this.autoCompleteTextbox.ClientID + @"')
                                                    .autocomplete(" + this.ClientID + @"_autocompleteData, 
                                                        {
                                                            mustMatch:true, 
                                                            autoFill:true, 
                                                    ...
                                                    .result(function (event, item)
                                                        { 
                                                            if (item) jQuery('#" + this.selectedIdHiddenField.ClientID + @"').val(item.id); 
                                                        });
                                            });
                                        </script>";

			ScriptManager.RegisterStartupScript(this, typeof(AutoCompleteDataEditorUpdatePanel), this.ClientID + "_autocomplete", jsAutocomplete, false);
		}
		...
		private void BuildAutoCompletePageScript()
		{
			...
			foreach (KeyValuePair<int, string> keyValuePair in this.sourceItems)
			{
				if (keyValuePair.Value != null)
				{
					...
					autocompleteData += "text: '" + keyValuePair.Value.Replace(delimiter, string.Empty).Replace("'", "\\'") + "',";
			...
			this.Page.ClientScript.RegisterClientScriptBlock(autocompleteData.GetType(), this.ClientID + "_autocompleteData", autocompleteData);
