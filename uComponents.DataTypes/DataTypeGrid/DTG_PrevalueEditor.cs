...
namespace uComponents.DataTypes.DataTypeGrid
{
	...
	public class PrevalueEditor : uComponents.DataTypes.Shared.PrevalueEditors.AbstractJsonPrevalueEditor
	{
			...
			: base(dataType, umbraco.cms.businesslogic.datatype.DBTypes.Ntext)
		{
			...
			DtgHelpers.EnsureFileExists(dtgFolder.FullName + "\\PreValueWebService.asmx", DtgWebServices.PreValueWebService, m_Locker);
		}
		...
		protected override void OnInit(EventArgs e)
		{
			...
			this.RegisterEmbeddedClientResource("uComponents.DataTypes.DataTypeGrid.Css.DTG_PrevalueEditor.css", ClientDependencyType.Css);
			this.RegisterEmbeddedClientResource("uComponents.DataTypes.DataTypeGrid.Scripts.DTG_PrevalueEditor.js", ClientDependencyType.Javascript);
		}
		...
		protected override void CreateChildControls()
		{
			...
			this._numberOfRows = new TextBox()
									 {
										 ID = "NumberOfRows",
										 Text = this._settings.NumberOfRows.ToString()
									 };
			this._numberOfRowsValidator = new RegularExpressionValidator()
											  {
												  ...
												  ErrorMessage = uQuery.GetDictionaryItem("MustBeANumber", "Must be a number")
											  };
			...
			{
				InnerText = uQuery.GetDictionaryItem("AddNewDataType", "Add new datatype")
			};
			...
			var valNewName = new RequiredFieldValidator()
				{
					...
					Display = ValidatorDisplay.Dynamic,
					ErrorMessage = uQuery.GetDictionaryItem("YouMustSpecifyAName", "You must specify a name")
				};
			...
			var valNewAlias = new RequiredFieldValidator()
			{
				...
				Display = ValidatorDisplay.Dynamic,
				ErrorMessage = uQuery.GetDictionaryItem("YouMustSpecifyAnAlias", "You must specify an alias")
			};
			...
			var valNewAliasExists = new CustomValidator()
			{
				...
				ErrorMessage = uQuery.GetDictionaryItem("AliasAlreadyExists", "Alias already exists!")
			};
		...
		public void GetConfig()
		{
			...
			this._newPreValue = new PreValueRow() { Id = DtgHelpers.GetAvailableId(this.m_DataType.DataTypeDefinitionId) };
		...
		private BasePreValueRow ParsePrevalue(PreValueRow t)
		{
			...
			{ 
				// Get values
				var name = t.Controls[0] != null
								? ((TextBox)t.Controls[0]).Text 
								: null;
				var alias = t.Controls[1] != null
								? ((TextBox)t.Controls[1]).Text
								: null;
				var dataTypeId = t.Controls[2] != null
								? int.Parse(((DropDownList)t.Controls[2]).SelectedValue)
								: 0;
				var validation = t.Controls[3] != null
								? ((TextBox)t.Controls[3]).Text
								: null;
				...
				var contentSortOrder = t.Controls[5] != null
								...
								: string.Empty;
				var sortOrder = t.Controls[6] != null 
								? int.Parse(((HiddenField)t.Controls[6]).Value) 
								: 0;
		...
		private void AddPrevalues(IList s)
		{
			foreach(var config in s)
			{
				...
				{
					uQuery.MakeNewPreValue(this.m_DataType.DataTypeDefinitionId, json, string.Empty, ((BasePreValueRow)config).SortOrder);
				}
			}
		}
		...
		private bool ContainsPreValue(IList s, string alias)
		{
			foreach (var config in s)
			{
				...
				if (!config.GetType().Name.Equals("PreValueEditorSettings") && ((BasePreValueRow) config).Alias.Equals(alias))
				{
					return true;
				}
			}
		...
		private string GetContentSorting(IList<PreValueRow> s)
		{
			var list =
				s.Where(
					...
					!string.IsNullOrEmpty(storedConfig.ContentSortOrder)).OrderBy(storedConfig => storedConfig.ContentSortPriority);
