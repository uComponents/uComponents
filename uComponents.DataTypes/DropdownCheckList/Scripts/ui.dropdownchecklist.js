	...
	$.widget("ui.dropdownchecklist", {
		...
		_syncSelected: function(senderCheckbox) {
			...
			allCheckboxes.each(function(index) {
				...
				var boolVal = (val === "checked" || val === "selected");
				$(selectOptions[index]).attr("selected", boolVal);
			});
		...
		_setSize: function(dropCalculatedSize) {
			...
			controlWrapper.find(".ui-dropdownchecklist-text").css('min-width', '66px');
			controlWrapper.find(".ui-dropdownchecklist-text").css('padding', '0 5px');
		...
		_init: function() {
			...
			// edit: make sure only one instance of the wrapper exists
			if ($(self.element).siblings(".ui-dropdownchecklist-wrapper").length > 0) {
				return;
			}   
