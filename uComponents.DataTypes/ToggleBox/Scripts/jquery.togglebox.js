	...
	$.fn.checkToggle = function (settings) {
		...
		function toggle(element) {
			...
			if (checked) {
				...
				$(element).animate({ marginLeft: '34px' }, 500,
				...
				$(element).animate({ marginLeft: '0px' }, 500,
		...
		return this.each(function () {
			...
			if ($(this).is(':checked')) {
				$(this).after('<div class="toggleSwitch"><span class="leftLabel">' + settings.on_label + '<\/span><div class="switchArea" style="background-color: ' + settings.on_bg_color + '"><span class="switchHandle left" style="margin-left:0px;"><\/span><\/div><span class="rightLabel" style="color:#cccccc">' + settings.off_label + '<\/span><\/div>');
			} else {
